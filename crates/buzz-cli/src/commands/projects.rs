...
async fn cmd_change_related_channels(
    ...
    let builder = build_project_related_channel_change(&coordinate, revision, add, remove)
...
mod tests {
    ...
    #[test]
    fn project_state_requires_relay_signature_coordinate_and_canonical_body() {
        ...
        .is_err());
        assert!(parse_project_state(
            project_state_event(&relay, &coordinate, "7", &format!(" {content}")),
            &relay.public_key().to_hex(),
            &coordinate,
        )
        .is_err());
    ...
    #[test]
    fn project_owner_defaults_to_delegating_owner() {
        let actor = Keys::generate();
        let owner = Keys::generate().public_key().to_hex();
        let auth = Tag::parse(["auth", &owner, "kind=47010", &"3".repeat(128)]).expect("auth tag");
        let client =
            crate::client::BuzzClient::new("http://127.0.0.1:9".into(), actor, Some(auth), None)
                .expect("client");
        assert_eq!(project_owner(&client, None).expect("owner"), owner);
    }

    #[test]
    fn project_change_cas_conflict_maps_to_exit_five() {
        let error = map_project_change_submit(Err(CliError::Relay {
            status: 400,
            body: "conflict: Project revision is 8".into(),
        }))
        .expect_err("conflict");
