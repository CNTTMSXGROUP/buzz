...
- `sdk.layout.save({dashboard, pan, widgets})` — persists the user's widget arrangement for one dashboard. Send only the widgets that differ from their `data/*.json` position and `pan: null` when it matches the package default; the host replays the overrides as `layouts` on `host.init`. No capability, debounced, and a wholesale replace per dashboard.
