...
async fn run_relay_main(boot: BootTracker) -> anyhow::Result<()> {
    ...
    match &tracer_init {
        ...
        telemetry::TracerInit::Disabled => tracing_init.skip(LifecycleReason::NotConfigured),
    ...
    let startup_heartbeat = boot.finish().spawn_refresh(usage_idle_timeout_secs);
    ...
    startup_heartbeat.shutdown().await;
