...
- Widget placement the user changes by dragging or nudging is persisted by the host, not by the package: call `sdk.layout.save({dashboard, pan, widgets})` after a drag, nudge, pan, or reset, passing only the widgets that differ from their `data/*.json` position (`pan: null` when it is the default). The host replays them as `layouts` on `host.init`, keyed by dashboard and widget id, so seed positions from `layouts[dashboardId]` before falling back to the package. It needs no capability, sends are debounced, and untouched widgets keep following the package, so a later revision can still move them.
