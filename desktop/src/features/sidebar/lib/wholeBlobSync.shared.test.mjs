...
// ── P2a queue-until-bootstrap regressions ────────────────────────────────────
//
// Three causal regressions covering the queue-until-bootstrap contract.
//
// Shared mutation target A (tests 1 and 2): removing the `bootstrapStarted`
// guard from `publish()` lets the debounce timer fire while bootstrap is still
// in-flight. `publishBaseline` is `{0,""}` at that point, so
// `remoteAdvancedSince(head, {0,""})` = true and `fetchOwnBlobBeforePublish`
// adopts the edit away — exactly the Kalvin P2a click-loss.
//
// Shared mutation target B (test 3): removing the `bootstrapFailed` exception
// from `fetchOwnBlobBeforePublish` causes a hard-adopt against `{0,""}` when
// bootstrap failed, dropping the queued edit even though no baseline was ever
// established.

// Test 1 (load-bearing): fresh device (watermark=0), relay HAS a pre-existing
// head, user clicks during unresolved bootstrap → bootstrap succeeds → edit
// publishes above the relay head, never adopted away.
// Failing mutation: remove `this.bootstrapStarted &&` from the publish() guard
// (i.e. revert to unconditional timer scheduling) → debounce fires against
// {0,""} baseline → remoteAdvancedSince returns true → adopt fires instead.
test("P2a queue-until-bootstrap: click during unresolved bootstrap on fresh device publishes above the relay head, never adopted", async () => {
  let releaseBootstrap = null;
  let publishCalls = 0;
  const { fireDelay, restore } = makeTimerBed();
  const remoteStore = makeStore([{ id: "remote", name: "Remote", order: 0 }]);
  const tauri = installEchoTauri("pk-p2a-1");

  // storedHead tracks what the relay retains. Initially the pre-existing head;
  // updated to our published event once we publish successfully.
  let storedHead = [tauri.mintHead(remoteStore, 200, "evt-relay-head")];

  // Fetch call 1 (bootstrap): blocks until released, returns the pre-existing
  // relay head. Subsequent calls (pre-publish fetch, confirmation) return
  // storedHead — which after our publish will be our own event.
  let fetchCalls = 0;
  mock.method(relayClient, "fetchEvents", () => {
    fetchCalls++;
    if (fetchCalls === 1)
  ...
  try {
    const manager = new SyntheticWholeBlobManager("pk-p2a-1", RELAY);
    const adopted = [];
    manager.setOnRemoteAdopted((r) => adopted.push(r));

    // Start bootstrap (blocked on fetch call 1).
    const bootstrapPromise = manager.bootstrap(nonEmptyStore());

    // User clicks during unresolved bootstrap. With the fix: no timer fires yet.
    // Without the fix (mutation): timer fires immediately against {0,""} baseline.
    manager.publishSections(
      makeStore([{ id: "click", name: "Click", order: 0 }]),
    );

    // Release bootstrap → head is recorded (200,"evt-relay-head"), bootstrapResolved=true,
    // releaseDeferred() re-freezes publishBaseline to {200,"evt-relay-head"} and
    ...
    // Fire the debounce released by bootstrap.
    await fireDelay(2000);
    for (let i = 0; i < 100; i++) await Promise.resolve();

    // The edit must have published. publishBaseline was {200,"evt-relay-head"};
    // pre-publish fetch returns the same head → remoteAdvancedSince = false → publish.
    // After our publish, storedHead = our event; confirmation fetch confirms us.
    // Without the fix: baseline was {0,""} → remoteAdvancedSince = true → adopt.
    assert.equal(adopted.length, 0, "click must not be adopted away");
    assert.ok(publishCalls >= 1, "click must publish above the relay head");
    assert.equal(
      manager.getPendingStore(),
      null,
...
// Test 2: click during unresolved bootstrap, bootstrap succeeds, then a peer
// posts a genuinely newer head BEFORE the debounce fires → edit adopts (normal LWW).
// Failing mutation: same as test 1 — without the guard, the debounce fires during
// bootstrap with baseline {0,""}. The pre-publish fetch returns a {0,""}-relative
// advance but for the wrong reason (before the bootstrap head was established).
// With the fix, publishBaseline = {200,"evt-boot"} and the peer head at ts=400
// is a genuine advance → adopt fires correctly.
test("P2a queue-until-bootstrap: genuinely newer peer head that arrives after bootstrap adopt via normal LWW", async () => {
  let releaseBootstrap = null;
  let publishCalls = 0;
  const { fireDelay, restore } = makeTimerBed();
  const bootstrapStore = makeStore([{ id: "boot", name: "Boot", order: 0 }]);
  const peerStore = makeStore([{ id: "peer", name: "Peer", order: 0 }]);
  const tauri = installEchoTauri("pk-p2a-2");

  const bootHead = tauri.mintHead(bootstrapStore, 200, "evt-boot");
  const peerHead = tauri.mintHead(peerStore, 400, "evt-peer");

  // Bootstrap returns the boot head. Pre-publish fetch returns the peer-newer
  // head (simulating a peer publishing between bootstrap and doPublish).
  let fetchCalls = 0;
  mock.method(relayClient, "fetchEvents", () => {
    fetchCalls++;
    if (fetchCalls === 1)
  ...
  try {
    const manager = new SyntheticWholeBlobManager("pk-p2a-2", RELAY);
    const adopted = [];
    manager.setOnRemoteAdopted((r) => adopted.push(r));

    const bootstrapPromise = manager.bootstrap(nonEmptyStore());

    // Click during unresolved bootstrap.
    manager.publishSections(
      makeStore([{ id: "click-2", name: "Click", order: 0 }]),
    );

    while (releaseBootstrap === null) await Promise.resolve();
    releaseBootstrap();
    await bootstrapPromise;
    // After bootstrap: publishBaseline = {200,"evt-boot"} (re-frozen by releaseDeferred).
    for (let i = 0; i < 50; i++) await Promise.resolve();

    await fireDelay(2000);
    for (let i = 0; i < 100; i++) await Promise.resolve();

    // Pre-publish fetch returns {400,"evt-peer"}: remoteAdvancedSince({400,...},{200,...})=true
    // → adopt fires correctly. Normal LWW: peer truly posted after bootstrap baseline.
    assert.equal(
      publishCalls,
      0,
      "edit must not publish over the newer peer head",
    );
    assert.equal(
      adopted.length,
      1,
      "newer peer head must be adopted (normal LWW)",
    );
    manager.destroy();
  } finally {
    tauri.restore();
...
// Test 3 (Kalvin's original P2a sequence): bootstrap FAILS, user clicks,
// pre-publish fetch finds a pre-existing head → edit publishes above it.
// Failing mutation: remove the `this.bootstrapFailed &&` check from
// fetchOwnBlobBeforePublish — the hard-adopt against {0,""} baseline fires.
test("P2a failed bootstrap: click after failed bootstrap publishes above the relay head, never adopts against empty baseline", async () => {
  let publishCalls = 0;
  const { fireDelay, restore } = makeTimerBed();
  const relayStore = makeStore([{ id: "relay", name: "Relay", order: 0 }]);
  const tauri = installEchoTauri("pk-p2a-3");

  // storedHead starts as the pre-existing relay head; updated to our event on publish.
  let storedHead = [tauri.mintHead(relayStore, 100, "evt-existing")];

  // Bootstrap fetch: call 1 rejects. Subsequent calls: return storedHead.
  let fetchCalls = 0;
  mock.method(relayClient, "fetchEvents", () => {
    fetchCalls++;
    if (fetchCalls === 1) return Promise.reject(new Error("relay timeout"));
    return Promise.resolve(storedHead);
  });
  mock.method(relayClient, "publishEvent", (e) => {
    publishCalls++;
  ...
  try {
    const manager = new SyntheticWholeBlobManager("pk-p2a-3", RELAY);
    const adopted = [];
    manager.setOnRemoteAdopted((r) => adopted.push(r));

    // Bootstrap fails: bootstrapFailed=true, bootstrapResolved=true.
    // No releaseDeferred edit to schedule (nothing was queued during bootstrap).
    await manager.bootstrap(nonEmptyStore());

    // User clicks after failed bootstrap — publish() sees bootstrapResolved=true
    // so it schedules immediately with publishBaseline={0,""} (lastRemoteHead
    // was never advanced because bootstrap failed).
    manager.publishSections(
      makeStore([{ id: "click-3", name: "Click", order: 0 }]),
    );

    await fireDelay(2000);
    for (let i = 0; i < 100; i++) await Promise.resolve();

    // Pre-publish fetch returns {100,"evt-existing"}: remoteAdvancedSince(true).
    // bootstrapFailed+empty-baseline exception: fold the head in and publish
    // above it instead of adopting. After publish, storedHead = our event;
    // confirmation confirms us.
    // Without the fix: hard-adopt fires, publishCalls=0, adopted.length=1.
    assert.equal(
      adopted.length,
      0,
      "click must not be adopted against empty baseline",
    );
    assert.ok(publishCalls >= 1, "click must publish above the relay head");
    assert.equal(
      manager.getPendingStore(),
