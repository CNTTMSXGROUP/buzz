...
fn wait_for_startup_metrics(process: &mut RelayProcess, port: u16) -> String {
    let deadline = Instant::now() + Duration::from_secs(8);
    loop {
        assert!(
            process.try_wait().is_none(),
            "relay exited before its startup receipt was scraped"
        );
        if let Ok(response) = scrape_metrics(port) {
            if response.contains("buzz_startup_phase_terminal") {
                return response;
            }
        }
        assert!(
            Instant::now() < deadline,
            "startup metrics did not become scrapeable within 8s"
        );
...
fn invalid_config_terminalizes_at_main_even_with_logs_disabled() {
    ...
    assert_terminal(&events, "config_load", "failed", Some("parse"));
    assert_terminal(&events, "process_telemetry", "failed", Some("parse"));
...
fn otlp_build_failure_is_degraded_without_leaking_endpoint_credentials() {
    ...
    let scrape = wait_for_startup_metrics(&mut process, port);
    assert!(scrape.contains(
        "buzz_startup_phase_terminal{phase=\"process_telemetry\",result=\"degraded\"} 1"
    ));
...
#[test]
fn successful_main_exports_the_two_current_startup_receipts() {
    let fake_database = TcpListener::bind(("127.0.0.1", 0)).expect("bind fake database");
    let database_url = format!(
        "postgres://buzz@127.0.0.1:{}/buzz",
        fake_database.local_addr().expect("database address").port()
    ...
    let scrape = wait_for_startup_metrics(&mut process, port);
    assert!(scrape.contains("# TYPE buzz_startup_phase_terminal gauge"));
    assert!(scrape
        .contains("buzz_startup_phase_terminal{phase=\"process_telemetry\",result=\"success\"} 1"));
    assert!(scrape.contains("# TYPE buzz_startup_phase_duration_seconds gauge"));
    assert_eq!(
        scrape
            .lines()
    ...
    assert!(!scrape.contains("process_boot_id"));
    assert!(!scrape.contains("reason="));
    assert!(!scrape.contains("phase=\"crypto_init\""));

    let second = scrape_metrics(port).expect("second metrics scrape");
    assert!(second
        .contains("buzz_startup_phase_terminal{phase=\"process_telemetry\",result=\"success\"} 1"));
    ...
    assert_terminal(&events, "tracing_init", "skipped", Some("not_configured"));
